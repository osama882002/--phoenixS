        ...
        <div class="bg-red-100 text-red-700 p-4 rounded mb-6">
            ...
            <ul class="mt-2 list-disc pr-6">
                @foreach (auth()->user()->unreadNotifications as $notification)
                    <li>
                        <span class="text-sm">
                            مقالة جديدة: <strong>{{ $notification->data['title'] }}</strong> بواسطة {{ $notification->data['author'] }}
                        </span>
                    </li>
                @endforeach
            </ul>
        ...
        <div class="space-y-6">
            @foreach($pendingPosts as $post)
                <div class="bg-white p-4 rounded shadow">
                    ...
                    <p class="text-sm text-gray-600 mb-2">بواسطة: {{ $post->user->name }} | القسم: {{ $post->category->name }}</p>
                    ...
                    @if($post->media_path)
                        <div class="mb-2">
                            ...
                            @else
                                <img src="{{ asset('storage/' . $post->media_path) }}" alt="media" class="rounded w-full">
                            @endif
                        </div>
                    @endif
