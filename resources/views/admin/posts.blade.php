                ...
                <table class="min-w-full text-sm text-gray-700 dark:text-gray-200">
                    ...
                    <tbody>
                        @foreach ($posts as $post)
                            <tr
                                class="border-b dark:border-gray-600 text-center hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                                ...
                                <td class="py-3 px-6">
                                    @php
                                        ...
                                        $statusInfo = $statusMap[$post->status] ?? [
                                            'label' => $post->status,
                                            'class' => 'text-gray-500',
                                        ];
                                    @endphp
                                ...
                                <td class="py-3 px-6 flex justify-center gap-3">
                                    ...
                                    @elseif ($post->status === 'pending')
                                        <a href="{{ route('admin.posts.review') }}"
                                            class="text-amber-600 dark:text-amber-400 text-sm hover:underline">👁️ عرض</a>
                                    @else
